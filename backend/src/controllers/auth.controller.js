...
export async function sendVerifyOtp(req, res) {
  try {
    ...
    await pool.query(
      `UPDATE users SET verify_otp = $1, verify_otp_expire = $2 WHERE id = $3`,
      [otp, expiry, userId]
    );
...
export async function verifyOtp(req, res) {
  try {
    ...
    await pool.query(
      `UPDATE users SET is_account_verified = true, verify_otp = NULL, verify_otp_expire = NULL WHERE id = $1`,
      [userId]
    );
...
export async function sendResetOtp(req, res) {
  try {
    ...
    await pool.query(
      `UPDATE users SET reset_otp = $1, reset_otp_expire = $2 WHERE email = $3`,
      [otp, expiry, email]
    );
...
export async function resetPassword(req, res) {
  try {
    ...
    await pool.query(
      `UPDATE users SET password = $1, reset_otp = NULL, reset_otp_expire = NULL WHERE email = $2`,
